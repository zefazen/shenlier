kecepatan = 35;kecepatan2=kecepatan;ftganti=0;fungsi=0;ftfungsi=0;fungsiAwal=0;fungsitimer=0;vketikhalo=halo.innerHTML;halo.innerHTML = "";pesanwhatsapp = pesanWA.innerHTML;deffotostiker=fotostiker.src;Content.style = "opacity:1;margin-top:30vh;";wallpaper.style="opacity:1";
  ...
  function bqmuncul(){bq.style = "position:relative;opacity:1;visibility:visible;transform: scale(1);margin-top:15px";fungsi=1;setTimeout(function(){setInterval(updateProgressBar, 100);},300);opsLclick=1;setTimeout(otokal,30);}
  
  const blockquote = document.getElementById('bq');
  function autoScroll() {
    const scrollHeight = blockquote.scrollHeight; // Total tinggi konten
    const currentScroll = blockquote.scrollTop; // Posisi scroll saat ini
    const clientHeight = blockquote.clientHeight; // Tinggi elemen yang terlihat

    const paddingTop = 50;

    if (currentScroll + clientHeight - paddingTop < scrollHeight) {
      blockquote.scrollTop += 100;
    } else {
  ...
  function prbhn(){if (opsLcheck == jumlahP - 1) {kecepatan=kecepatan2;var prg = document.getElementById("kalimat");prg.classList.add("kiri");kalimat.style="font-size:14px"}}
  ...
  afterComplete: function(){
    ...
    if (opsLcheck !== jumlahP) {setInterval(updateProgressBar, 100);opsLclick=1;/*otopsL();*/} else {ftganti += 1;fthilang();setTimeout(ftmuncul, 300);setInterval(berjatuhan,200);/*tombol()*/}
  },}).go();
  }
