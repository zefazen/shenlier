...
function hatiJatuh() {
  ...
  hati.className = 'hati';
  hati.innerHTML = `<svg class="line" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8Z" fill="currentColor"/></svg>`;
...
function bukaEnvelope() {
  ...
  function mulaiHal2() {
    ...
    // setTimeout(selesaiGame(true), 3000); // Untuk sementara karna kita fokus ke hal3
  ...
  function balikKartu(elemenKartu, card, index) {
    ...
    function selesaiGame(sukses) {
      gameJalan = false;
      if (sukses) {
        setTimeout(()=>{document.getElementById(`hal2`).style = "transform:scale(0);transition:all .6s ease";setTimeout(()=>{pindahHal(4)}, 450);}, 730);
      } else {
    ...
    // Fungsi Buka Kado (update ini saja)
    function bukaKado() {
      const giftBox = document.getElementById('giftBox');
      const container = giftBox.parentElement;  // yaitu .gift-container
      giftBox.classList.add('open');
      container.classList.add('open');          // tambahkan ini
      tombolBuka.style="transform: scale(.75); opacity: .3; transition: all 0.5s ease";
      // setTimeout(() => {
    ...
    function pindahHal(hal) {
      for (let i = 1; i <= 4; i++) {
        if (hal < 5) document.getElementById(`hal${i}`).classList.add('sembunyi');
      }
      if (hal < 5) document.getElementById(`hal${hal}`).classList.remove('sembunyi');
      
      setTimeout(()=>{
         if (hal === 2) {mulaiHal2();document.getElementById(`hal${hal}`).style="width:90%;margin-top:100px;transform:scale(1);transition:all .7s ease";}
         if (hal !== 2){
         document.getElementById(`hal${hal - 1}`).style = "transform:scale(0);transition:all .6s ease";
         if (hal == 4){setTimeout(()=>{document.getElementById(`hal${hal}`).style="width:90%;padding:.9rem;margin-top:100px;transform:scale(1);transition:all .7s ease";}, 100);}
         setTimeout(()=>{
         	document.getElementById(`hal${hal - 1}`).classList.add('sembunyi');
             document.getElementById(`hal${hal}`).classList.remove('sembunyi');
	         setTimeout(()=>{document.getElementById(`hal${hal}`).style="width:90%;padding:.9rem;margin-top:100px;transform:scale(1);transition:all .7s ease";}, 100);
	         setTimeout(()=>{if (hal === 4) mulaiHal4();}, 0);
	     }, 300);
	     }
      }, 50);
    }
    
    function kirimPesan(pilihan) {
      let text = "";
      if (pilihan === "mau") {
        text = "Ayoo mabar 🫢";
      } else if (pilihan === "gamau") {
        text = "Maaf beb, ngga sekarang..";
      }
